...
func (s *testScanSuite) TestScan() {
	...
	for _, rowNum := range s.rowNums {
		...
		if rowNum > 123 {
			_, err = s.store.SplitRegions(context.Background(), [][]byte{s.makeKey(123)}, false, &mockTableID, false)
			s.Nil(err)
		}

		if rowNum > 456 {
			_, err = s.store.SplitRegions(context.Background(), [][]byte{s.makeKey(456)}, false, &mockTableID, false)
			s.Nil(err)
		}
